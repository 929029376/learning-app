...
- 渲染 Markdown 章节笔记，并支持更灵活的小节笔记匹配。
...
Markdown 文件支持 `.md`、`.markdown` 和 `.mdown` 后缀。

应用会优先从常见课程资料目录中寻找章节笔记，例如：
...
在把 Markdown 笔记匹配到某个小节时，应用也会扫描所选学习资料根目录下的 Markdown 候选文件。`.git`、`.learning-data`、`node_modules`、`dist`、`build` 等生成目录或依赖目录会被忽略。

为了让笔记稳定关联到某个小节，建议在文件名、路径或标题中写出小节编号或阶段 id，例如：
...
如果多个小节共用一个课程笔记文件，可以用包含小节标识的标题分段：
...
应用找到匹配标题后，会显示该标题到下一个同级或更高级标题之间的内容。新增、移动或重命名学习资料后，请在应用中执行扫描/刷新，让课程索引和小节关联重新生成。
